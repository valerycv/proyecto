...
int open_database() {
    ...
    printf("Tabla products verificada o creada exitosamente.\n");  // Mensaje de depuración
    return 0;
}
...
int add_product(Product *product) {
    ...
    if (get_product_by_id(product->id, &existing_product) == 0) {
        ...
        return -1; // Retorna -1 si el ID ya está en uso
    }
...
int delete_product(int id) {
    ...
    // Vincular el ID a la declaración
...
int get_product_by_id(int id, Product *product) {
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        // Validar que las columnas no sean nulas
        ...
        // Asignar valores a la estructura del producto
